...
int oi_putchar(int c) {
    return putchar(c);
}

FILE* oi_fopen(const cstr* filename, const cstr* mode) {
    return fopen(filename, mode);
}
int oi_fclose(FILE* stream) {
    return fclose(stream);
}



struct stdoi_conspec;
struct stdoi_conspec stdoi_parse_conspec(char**);
/*  
...
enum stdoi_conspec_obj_types {NO_INFO = 0, HH, H, L, LL, J, Z, T};

int oi_fprintf(FILE* stream, const cstr* fmt, ...) {
    va_list args;
    va_start(args, fmt);
    
    int n;
    struct stdoi_conspec conspec;
    while(*fmt) {
        n++;          // character count
        if(!(*fmt))
            return n;
        ...
        conspec = stdoi_parse_conspec((char**) &fmt);
 ...
 *            '''''''''''''''''''''''''''''''''''''''''''''''''''''
*/
struct stdoi_conspec stdoi_parse_conspec(char **cursor_ptr) {
    char *cursor = *cursor_ptr;
    struct stdoi_conspec conversion_spec = {0,0,0,0,0};
    int loop_count;
    State_One:
